...
| §8.279 | `delete_subst_param_names.py` | `ASTTypeSubstituter.param_names` (stored, never read) and the four callers' copies of it deleted, over `12481659` (13 asserted edits, 4 files); the lane gate's rule 1b that surfaced the type lives in `scripts/lane-gate.py` with its mutations in `scripts/lane-gate-selftest.py`, and audit 14's own instrument is not committed (its session's scratchpad; copied to `.objcmp/sa/nomap_owners_r14.py`) |
| §8.281 | `residue.md`, `residue.py`, `residue_classify.py` (at this directory's root, not under an entry: they are D32's living artifact, not one entry's instrument) | D32's residue: `residue.py` derives every name-keyed read in `compiler/src` from `scripts/lane-gate.py`'s own parser (`--count` the size, no flag the sites, `--check` the list against the tree by (file, method, key text) in both directions AND every row's class against `residue_classify.py`'s verdict, with the per-class counts from the classifier; `--elsewhere` the population names called on types that hold nothing); `residue_selftest.py` drives the check through a throwaway tree and list in both directions (`make residue-selftest`, in `check-fast`); `residue_classify.py` holds the class of each read method and the per-site overrides as code and writes `residue.md`'s table between its markers, refusing a stale class, an unclassified method or an override matching no call; a run that changes a row's class is a change of judgement and goes in the ledger |
