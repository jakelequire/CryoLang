...
namespace Cryo
{
    namespace LSP
    {
        ...
        std::string ProtocolHandler::createSuccessResponse(const JsonValue &id, const JsonValue &result)
        {
            ...
            JsonValue jsonResponse = response.toJson();
            std::string serialized = jsonResponse.toString();

            // Validate that the JSON is not empty and looks correct
            if (serialized.empty())
            {
                Logger::instance().error("LSP", "Created empty response JSON!");
                return createErrorResponse(id, LSPErrorCode::InternalError, "Empty response");
            }

            if (serialized.length() < 2 || serialized[0] != '{' || serialized.back() != '}')
            {
                Logger::instance().error("LSP", "Created malformed response JSON: " + serialized);
                return createErrorResponse(id, LSPErrorCode::InternalError, "Malformed response");
            }

            Logger::instance().debug("LSP", "Created response JSON: " + serialized);
            return serialized;
        }
