...
Lexer::Lexer(std::unique_ptr<File> file)
    ...
    , _current_token(TokenKind::TK_ERROR)
    , _previous_token(TokenKind::TK_ERROR)
    , _token_count(0) {
...
Token Lexer::lex_identifier() {
    ...
    return Token(kind, text, SourceLocation(_current_location.line(),
...
Token Lexer::lex_string(char quote) {
    ...
    while (!at_end() && peek() != quote) {
        if (peek() == '\\' && _current + 1 < _buffer_end) {
            ...
            if (!at_end()) advance(); // consume escaped character
        } else {
            if (peek() == '\n') {
...
TokenKind Lexer::lex_punctuator(char c) {
    switch (c) {
        case '(':  return TokenKind::TK_L_PAREN;
        case ')':  return TokenKind::TK_R_PAREN;
        case '{':  return TokenKind::TK_L_BRACE;
        case '}':  return TokenKind::TK_R_BRACE;
        case '[':  return TokenKind::TK_L_SQUARE;
        case ']':  return TokenKind::TK_R_SQUARE;
        case ';':  return TokenKind::TK_SEMICOLON;
        ...
        case '%':  return TokenKind::TK_PERCENT;
        case '^':  return TokenKind::TK_CARET;
        case '~':  return TokenKind::TK_TILDE;
        case '@':  return TokenKind::TK_AT;
        case '$':  return TokenKind::TK_DOLLAR;
        case '\\': return TokenKind::TK_BACKSLASH;
        default:   return TokenKind::TK_ERROR;
    }
}
...
#define TOK(X) case TokenKind::TK_##X: return #X;
#define KEYWORD(X) case TokenKind::TK_KW_##X: return "KW_" #X;
#define PUNCTUATOR(X, Y) case TokenKind::TK_##X: return #X;
#define DIRECTIVE(X) case TokenKind::TK_PP_##X: return "PP_" #X;
#define ATTRIBUTE(X) case TokenKind::TK_ATTR_##X: return "ATTR_" #X;
...
const char* get_token_spelling(TokenKind kind) {
    switch (kind) {
#define TOK(X)
#define KEYWORD(X) case TokenKind::TK_KW_##X: return #X;
#define PUNCTUATOR(X, Y) case TokenKind::TK_##X: return Y;
#define DIRECTIVE(X)
#define ATTRIBUTE(X)
#include "Lexer/tokens.def"
#undef TOK
