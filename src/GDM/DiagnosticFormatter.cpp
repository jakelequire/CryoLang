...
namespace Cryo
{
    ...
    std::string DiagnosticFormatter::generate_enhanced_inline_label(ErrorCode error_code, const std::string &message) const
    {
        ...
        case ErrorCode::E0201_UNDEFINED_VARIABLE: // No field exists on type
            // Don't truncate or lowercase type names for undefined variable errors
            if (message.find("No field") != std::string::npos)
            {
                return message; // Return the full message to preserve type names
            }
